...
class RobotsCache:
    ...
    async def _load_robots(self, base_url: str, user_agent: str):
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning: Could not fetch robots.txt for {base_url}: {e}[/yellow]"
            )
            rp.parse([])
...
class WebCrawler:
    def __init__(self, start_urls: List[str], config: Dict):
    ...
    def _init_language_detector(self):
        ...
        if not os.path.exists(model_path):
            console.print(
                f"[bold red]Error: Language detection model '{model_path}' does not exist.[/bold red]"
            )
            return None
        try:
    ...
    async def _init_database(self):
        ...
        if not database_url:
            console.print(
                "[yellow]Warning: DATABASE_URL not set. Skipping database storage.[/yellow]"
            )
        ...
        try:
            self.db_pool = await asyncpg.create_pool(
                database_url, ssl=ssl_context, min_size=1, max_size=10
            )
            ...
            if os.path.exists(schema_path):
                ...
                console.print(
                    f"[bold red]Error: schema.sql not found at {schema_path}.[/bold red]"
                )
                self.save_to_db = False
            ...
            if os.path.exists(migration_path):
                ...
                async with self.db_pool.acquire() as conn:
                    try:
                        ...
                        console.print(
                            "[green]✓ Links migration applied successfully[/green]"
                        )
                    except Exception as e:
    ...
    async def _store_raw_html_in_cdn(
            ...
            try:
                async with self.db_pool.acquire() as conn:
                    async with conn.transaction():
                        ...
                        await conn.execute(
                            "CREATE TEMP TABLE temp_links (source_url text, target_url text) ON COMMIT DROP"
                        )
                        await conn.copy_records_to_table(
                            "temp_links", records=local_link_batch
                        )
    ...
    async def _add_to_db_batch(self, page_data: Dict, outgoing_links: List[str]):
        ...
        if (
            len(self.db_batch) >= self.db_batch_size
            or len(self.link_batch) >= self.db_batch_size * 2
        ):
    ...
    async def _fetch_via_renderer(self, url: str) -> Optional[str]:
        ...
        try:
            params = {"url": url}
            async with self.session.get(
                renderer_url, params=params, timeout=aiohttp.ClientTimeout(total=60)
            ) as response:
                if response.status == 200:
            ...
            async with self.session.get(
                ...
                if response.status == 200 and "text/html" in response.headers.get(
                    ...
                    if self._is_content_insufficient(html):
                        console.print(
                            f"[yellow]SPA detected for {url}, using JS Renderer...[/yellow]"
                        )
    ...
    async def _parse_page(
        self, url: str, html: str, depth: int, cdn_path: Optional[str]
    ) -> Tuple[Dict, List[str]]:
        soup = BeautifulSoup(html, "lxml")
        ...
        if self.lang_detector and len(body_text) > 50:
            try:
                ...
                pred = await loop.run_in_executor(
                    None, self.lang_detector.predict, clean, 1
                )
        ...
        async with semaphore:
            ...
            try:
                ...
                if res:
                    ...
                    page_data, links = await self._parse_page(
                        norm_url, html, depth, cdn_path
                    )
                ...
                if fails <= self.max_retries:
                    console.print(
                        f"[yellow]Error fetching {norm_url}: {e}. Retrying ({fails}/{self.max_retries})...[/yellow]"
                    )
                    await self.redis.srem("visited_urls", norm_url)
                    await self.redis.zadd("crawl_queue", {norm_url: depth})
                else:
                    console.print(
                        f"[bold red]Failed {norm_url} after {fails} attempts: {e}[/bold red]"
                    )
                    ...
                    if self.db_pool:
                        try:
                            ...
                            console.print(
                                f"[red]Failed to soft-delete {norm_url}: {dbe}[/red]"
                            )
        ...
        try:
            ...
            with Progress(
                ...
                while not self.shutdown_event.is_set():
                    ...
                    progress.update(
                        task,
                        description=f"Crawled: {self.stats['total_crawled']} | Links: {self.stats['links_recorded']}",
                    )
        finally:
            await self._flush_db_batch()
            if self.session:
    ...
    def print_summary(self):
        ...
        console.print(
            f"Pages: {self.stats['total_crawled']}, Links Recorded: {self.stats['links_recorded']}"
        )
...
async def load_crawler_config_from_db() -> Dict:
    ...
    try:
        ...
        val = await conn.fetchval(
            "SELECT value FROM crawler_config WHERE key = 'default'"
        )
        await conn.close()
        if val:
